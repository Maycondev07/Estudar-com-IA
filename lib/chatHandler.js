...
// - Google (Gemini): https://generativelanguage.googleapis.com/v1beta      modelo: "gemini-2.5-flash"
...
const API_MODEL = process.env.AI_API_MODEL || "gemini-2.5-flash";
const API_KEY = process.env.AI_API_KEY;

async function handleChat({ messages, mode, difficulty }) {
  if (!API_KEY) {
    const err = new Error(
  ...
  const stateContext = buildStateContext({ mode, difficulty });
